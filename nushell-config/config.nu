...
# nvimのvim.opt.shell = "nu -e $env.RUN_IN_NVIM = true" して区別する
$env.RUN_IN_NVIM = false

def nvim [...args : string] { 
  if ($env.RUN_IN_NVIM == false) {
    ^nvim ...$args
  } else {
    ^nvr -s --remote-tab ...$args
  }
}
...
# source "/Users/tsukimizake/Library/Application Support/nushell/rtx.nu"

def elm-review-fix [] { 
 yes | yarn --silent workspace frontend run --silent elm-review --config ../frontend-review/ --fix 
}

def elm-review-fix-debug [] { 
  yes | yarn --silent workspace frontend run --silent elm-review --config ../frontend-review/ --fix --debug 
}

def "nu-complete mask" [...args] {
  let root_commands = (^mask --introspect | from json).commands
  $root_commands | each {|command| nu_complete_mask_collect_subcommands $command } | flatten
}

def nu_complete_mask_collect_subcommands [parent_command] {
    let children = $parent_command.subcommands
    if $children != [] {
      $children
      | each {|child|  
                (nu_complete_mask_collect_subcommands $child) 
                  | each {|c|
                    $"($parent_command.name) ($c)"
                    }
            } 
            | flatten
            | append (if parent_command.script != null { [$parent_command.name] } else { [] })
    } else {
    if parent_command.script != null {
      [$parent_command.name]
    } else {
      []
    }
...
export extern mask [ ...args: string@"nu-complete mask" ]
